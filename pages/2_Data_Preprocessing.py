...
with st.container():
    st.subheader("数据预处理")
    single_tab,entirety_tab = st.tabs(['单列操作','自动化策略'])
    with single_tab:
        ...
        with col2:
            ...
            # 编码
                ...
                if data.loc[:,selected_column].nunique() < 20:
...
if submit:
    ...
    # 一般数据进行填补缺失值
        ...
        if select_fill_type == '众数':
            ...
            data.loc[:,selected_column] = imp.fit_transform(data.loc[:,selected_column].values.reshape(-1,1))
            write_cache_pt(data)
...
with st.container():
    ...
    with main_tab:
        dataframe_tab,datainfo_tab,msno_tab,heatmap_tab = st.tabs(["展示","概览","缺失矩阵","heatmap"])
        with dataframe_tab:
            df_empty = st.empty()
            ...
            #df_empty.empty()
        with datainfo_tab:
            df_info = pd.DataFrame(columns=['特征','数据类型','缺失值统计','值统计'])
            for each in data.columns.tolist():
                data_type = type(data.loc[:,each].unique().tolist()[0])
                miss_count = data.loc[:,each].isnull().sum()
                value_count = data.loc[:,each].nunique()
                df_temp = pd.DataFrame({'特征':each,'数据类型':data_type,'缺失值统计':miss_count,'值统计':value_count},index=[''])
                df_info = pd.concat([df_info,df_temp],axis=0)
            st.dataframe(df_info,use_container_width=True)
                         
        with msno_tab:
            p = msno.matrix(data.sample(data.shape[0]),figsize=(20,10),width_ratios=(15,1))
            st.pyplot(p.figure)
        with heatmap_tab:
            st.write(msno.heatmap(data.sample(data.shape[0]),figsize=(20,10)).figure)    
    ...
    with sub_tab:
        ...
        with column_tab:
            try:
                ...
                st.write('')
        with info_tab:
            st.write('type:',type(data.loc[:,selected_column].unique().tolist()[0]))
            st.write('missng value count:',data.loc[:,selected_column].isnull().sum())
        ...
        with col3:
            if save_type == 'local':
                ...
                if st.button("save on the platform"):
                    download_plantform(data,filename=save_filename)
                    st.success('saving sucessfully!')
